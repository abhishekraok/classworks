...
for ithmodel, mname in zip(*[modelset, modelnames]):  
    ...
    ErrorEI = np.average([i != j for i, j in zip(Ypredict[Ytest == 0], Ytest[Ytest == 0])])
    ErrorIE = np.average([i != j for i, j in zip(Ypredict[Ytest == 1], Ytest[Ytest == 1])])
    ErrorN  = np.average([i != j for i, j in zip(Ypredict[Ytest == 2], Ytest[Ytest == 2])])
